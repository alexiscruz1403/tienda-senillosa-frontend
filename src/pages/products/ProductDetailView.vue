<template>
  <app-layout>
    <div class="bg-[#F4F6FB] pb-8 relative">
      <v-breadcrumbs
        :items="[
          { title: 'Inicio', href: '/' },
      ...
      <section class="flex flex-col lg:flex-row gap-2 px-4 py-4" v-else>
        <div class="flex flex-col gap-4 md:gap-2 relative">
          ...
          <div class="flex gap-2">
            <div v-for="(image, index) in product?.images" :key="index">
              <img
                :src="image.url"
                :alt="`Imagen ${index + 2} de ${product?.name}`"
                :class="getSelectedImageClass(index)"
        ...
        <div class="flex flex-col gap-4 mt-8 md:mt-0">
          ...
          <div class="flex gap-2 items-center">
            ...
            <p v-if="product?.discount" class="text-xl line-through text-gray-500">
              ${{ product?.price }}
            </p>
          </div>
            ...
            <div class="flex flex-col gap-2">
              ...
              <div class="flex gap-3">
                <size-button
                  ...
                  :key="index"
                  :size="stock.size"
                  :selected="selectedSize == stock.size"
                  @click="handleSizeClick"
                />
              </div>
            </div>
      ...
      <section class="px-4 flex flex-col gap-4 mt-4">
        <h2 class="text-2xl md:text-3xl font-semibold">
          Productos <span class="text-[#3C83F6]">Similares</span>
        </h2>
        <div
...
const discountedPrice = computed(() => {
  if (product.value && product.value.discount) {
    return (product.value.price - (product.value.price * product.value.discount) / 100).toFixed(2)
  }
