<template>
  <div
    class="max-h-max relative shadow-md hover:shadow-lg transition-shadow bg-white rounded-lg overflow-hidden"
  >
    <div class="h-58 md:h-72 lg:h-80 bg-white relative group" @click="handleCardClick">
      ...
      <div class="flex flex-col gap-2 max-w-max absolute top-2 left-2">
        ...
        <v-chip v-if="props.discount" color="error" variant="flat" size="small"
          >{{ props.discount }}% OFF</v-chip
        >
      </div>
      <button
        class="absolute top-2 right-2 opacity-110! group-hover:opacity-100! transition-opacity duration-300"
    ...
    <div class="flex flex-col gap-2 px-2 py-4 bg-white">
      ...
      <div class="flex items-center gap-2">
        ...
        <span v-if="props.discount" class="text-sm line-through text-gray-500"
...
const likeClass = computed<string>(() => {
  ...
  return props.isLiked
    ? `${baseClass} bg-[#3C83F6] hover:bg-[#2a62d6] transition-colors duration-200`
    : `${baseClass} bg-gray-200 hover:bg-gray-400 transition-colors duration-200`
})
