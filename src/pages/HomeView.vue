  ...
  <app-layout>
    <section class="relative h-screen w-full">
      <v-carousel show-arrows="hover" hide-delimiters height="100vh">
        <template v-slot:prev="{ props }">
          <button class="bg-[#F8FAFC]/10 px-2 py-2 backdrop-blur-xs border rounded-full transition-colors text-white"
            @click="props.onClick">
            <ChevronLeft :size="30" />
          </button>
        </template>
        ...
        <v-carousel-item v-for="(item, i) in 5" :key="i">
          <img src="../../public/imgs/zapatillas-home.jpg" alt="Imagen zapatillas"
            class="w-full h-full object-cover absolute" />

          <div class="absolute inset-0 bg-linear-to-br from-[#0f1729] to-[#073c92] opacity-80 w-full h-full"></div>
          ...
          <div
            ...
            <h2 class="text-white text-5xl md:text-7xl font-bold">
              Zapatillas exclusivas
            </h2>
            <p class="text-white text-xl">Las zapatillas más buscadas ya están aquí. No te quedes sin las tuyas</p>
    ...
    <section class="bg-[#F4F6FB] py-10">
      <div class="flex flex-col gap-1 px-4 py-4">
        ...
        <div class="flex flex-col gap-2 md:flex-row md:justify-between md:items-center md:gap-0">
          <h2 class="text-3xl md:text-5xl font-semibold">Productos <span class="text-[#3C83F6]">Destacados</span></h2>
      ...
      <div class="relative w-full px-4">
        <div class="overflow-hidden">
          <div
            ...
            ref="productsCarousel" v-if="loading">
          ...
          <div
            ...
            <div class="px-2" v-for="product in featuredProducts" :key="product.product_id">
              <ProductCard :product_id="product.product_id" :name="product.name" :brand="product.brand"
                ...
                :discount="product.discount" @like="handleLikeClick" :isLiked="product.is_liked" />
            </div>
          </div>
        </div>

        <div class="flex w-full justify-between absolute top-1/2 -translate-y-1/2 left-0 px-4 pointer-events-none">
          <button @click="prevSlide" :disabled="disablePrevButton()"
          ...
          <button @click="nextSlide" :disabled="disableNextButton()"
            class="px-2 py-2 bg-white rounded-full hover:bg-[#3C83F6] hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed pointer-events-auto shadow-lg">
            <ChevronRight :size="30" />
          </button>
        </div>
    ...
    <section class="bg-[#F4F6FB] flex flex-col gap-8 py-10 px-4">
      <div class="flex flex-col gap-4 items-center">
        ...
        <h2 class="text-3xl md:text-5xl font-semibold">Más que <span class="text-[#3C83F6]">Ropa</span></h2>
        <p class="text-lg text-[#65758B] max-w-3xl text-center">
          Nacimos en 2020 con una misión clara: democratizar el streetwear en Argentina. Creemos que el estilo no tiene
          que
          ser caro para ser auténtico. Cada prenda que diseñamos está pensada para quienes se atreven a expresarse, para
          los
          que no siguen tendencias sino que las crean. Tienda Senillosa es más que una marca, es una comunidad de
          jóvenes
          que
          redefinen las reglas de la moda.
        </p>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <v-card color="white" variant="flat" class="py-6!">
          ...
          <v-card-subtitle class="text-center! text-wrap!">Materiales seleccionados para prendas que
            duran</v-card-subtitle>
        </v-card>
        <v-card color="white" variant="flat" class="py-6!">
          <div class="flex justify-center items-center">
            <div class="rounded-full bg-[#3C83F61A] px-3 py-3">
          ...
          <v-card-subtitle class="text-center! text-wrap!">Recibe tu pedido en cualquier punto de
            Argentina</v-card-subtitle>
        </v-card>
        <v-card color="white" variant="flat" class="py-6!">
          <div class="flex justify-center items-center">
            <div class="rounded-full bg-[#3C83F61A] px-3 py-3">
          ...
          <v-card-subtitle class="text-center! text-wrap!">Tus datos y compras están protegidas</v-card-subtitle>
        </v-card>
        <v-card color="white" variant="flat" class="py-6!">
          <div class="flex justify-center items-center">
            <div class="rounded-full bg-[#3C83F61A] px-3 py-3">
          ...
          <v-card-subtitle class="text-center! text-wrap!">Tienes 30 días para cambiar o devolver tu
            pedido</v-card-subtitle>
        </v-card>
      </div>
    </section>
  </app-layout>
